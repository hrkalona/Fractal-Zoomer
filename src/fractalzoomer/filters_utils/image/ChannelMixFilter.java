...
public class ChannelMixFilter extends PointFilter {
	...
	public int filterRGB(int x, int y, int rgb) {
		...
		int g = (rgb >> 8) & 0xff;
		int b = rgb & 0xff;
		int nr = PixelUtils.clamp(((255-intoR) * (blueGreen*g+(255-blueGreen)*b)/255 + intoR*r)/255);
		int ng = PixelUtils.clamp(((255-intoG) * (redBlue*b+(255-redBlue)*r)/255 + intoG*g)/255);
		int nb = PixelUtils.clamp(((255-intoB) * (greenRed*r+(255-greenRed)*g)/255 + intoB*b)/255);
