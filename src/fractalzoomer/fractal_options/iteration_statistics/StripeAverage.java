 ...
 * @author hrkalona2
 */
public class StripeAverage extends GenericStatistic {
    private double sum;
    private double sum2;
    private double StripeDensity;
    private double log_bailout_squared;

    public StripeAverage(double statistic_intensity, double StripeDensity, double log_bailout_squared, boolean useSmoothing, boolean useAverage) {
    ...
    public void insert(Complex z, Complex zold, Complex zold2, int iterations, Complex c, Complex start, Complex c0) {
        ...
        samples++;
        sum2 = sum;
        sum += 0.5 * Math.sin(StripeDensity * (z.arg() + Math.PI)) + 0.5;
    }
