...
public class UserConditionalPlaneInfluence extends PlaneInfluence {
    ...
    public UserConditionalPlaneInfluence(String[] user_function_filter_conditions, String[] user_function_filter_condition_formula, int max_iterations, double xCenter, double yCenter, double size, double[] point, Complex[] globalVars) {
        super();
    ...
    public Complex getValue(Complex z, int iterations, Complex c, Complex start, Complex zold, Complex zold2, Complex c0, Complex pixel) {
        ...
        if (resultParser.foundZ()) {
            resultParser.setZvalue(z);
        }

        if (resultParser.foundC()) {
            resultParser.setCvalue(c);
        }

        if (resultParser.foundS()) {
            resultParser.setSvalue(start);
        }

        if (resultParser.foundPixel()) {
            resultParser.setPixelvalue(pixel);
        }

        if (resultParser.foundC0()) {
            resultParser.setC0value(c0);
        }

        if (resultParser.foundP()) {
            resultParser.setPvalue(zold);
        }

        if (resultParser.foundPP()) {
            resultParser.setPPvalue(zold2);
        }
