...
public class SkewPlane extends Plane {
    ...
    public SkewPlane(double plane_transform_angle, double plane_transform_angle2, Apfloat[] center, Fractal f) {

        super();

        use_center = center[0].compareTo(Apfloat.ZERO) != 0 && center[1].compareTo(Apcomplex.ZERO) != 0;

        scales = new Complex(Math.tan(Math.toRadians(plane_transform_angle)), Math.tan(Math.toRadians(plane_transform_angle2)));
