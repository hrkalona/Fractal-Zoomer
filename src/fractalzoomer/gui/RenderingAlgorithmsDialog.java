...
import fractalzoomer.core.rendering_algorithms.CircularBruteForceRender;
...
public class RenderingAlgorithmsDialog extends JDialog {
    ...
    public RenderingAlgorithmsDialog(Component ptra, boolean greedy_algorithm, int greedy_algorithm_selection, int brute_force_alg, int guesses_selection) {
        ...
        circular_pattern.setSelectedIndex(TaskRender.CIRCULAR_COMPARE_ALG);
        circular_pattern.setFocusable(false);
        circular_pattern.setToolTipText("Sets the rendering pattern.");

        JTextField nnorm = new JTextField(6);
        nnorm.setText("" + TaskRender.CIRCULAR_N);
        ...
        revert_pattern.setSelected(TaskRender.CIRCULAR_REVERT_ALG);
        ...
        repeat_pattern.setSelected(TaskRender.CIRCULAR_REPEAT_ALG);
...
//        pattern_follows_zoom_center.setSelected(MainWindow.CIRCULAR_RENDER_FOLLOWS_ZOOM_TO_CURSOR);
  ...
  //                  pattern_follows_zoom_center.setEnabled(ptra instanceof MainWindow && circular_pattern.getSelectedIndex() != 4);
       ...
       // panel4.add(pattern_follows_zoom_center);
        ...
        ok.addActionListener(e -> {
            ...
            int oldCircularAlg = TaskRender.CIRCULAR_COMPARE_ALG;
            TaskRender.CIRCULAR_COMPARE_ALG = circular_pattern.getSelectedIndex();
            if(oldCircularAlg != TaskRender.CIRCULAR_COMPARE_ALG) {
                CircularBruteForceRender.clear();
                CircularBruteForceRender.clearFastJulia();
            }

            boolean oldRevertPattern = TaskRender.CIRCULAR_REVERT_ALG;
            TaskRender.CIRCULAR_REVERT_ALG = revert_pattern.isSelected();
            if(oldRevertPattern != TaskRender.CIRCULAR_REVERT_ALG) {
                CircularBruteForceRender.clear();
                CircularBruteForceRender.clearFastJulia();
            }

            boolean oldRepeatPattern = TaskRender.CIRCULAR_REPEAT_ALG;
            TaskRender.CIRCULAR_REPEAT_ALG = repeat_pattern.isSelected();
            if(oldRepeatPattern != TaskRender.CIRCULAR_REPEAT_ALG) {
                CircularBruteForceRender.clear();
                CircularBruteForceRender.clearFastJulia();
            }

            double oldNnorm = TaskRender.CIRCULAR_N;
            TaskRender.CIRCULAR_N = temp;
            if(TaskRender.CIRCULAR_N != oldNnorm) {
                CircularBruteForceRender.clear();
                CircularBruteForceRender.clearFastJulia();
            }

//            boolean oldFollowZoom = MainWindow.CIRCULAR_RENDER_FOLLOWS_ZOOM_TO_CURSOR;
//            MainWindow.CIRCULAR_RENDER_FOLLOWS_ZOOM_TO_CURSOR = pattern_follows_zoom_center.isSelected();
//            if(MainWindow.CIRCULAR_RENDER_FOLLOWS_ZOOM_TO_CURSOR != oldFollowZoom) {
//            CircularBruteForceRender.clear();
//            CircularBruteForceRender.clearFastJulia();
//            }
