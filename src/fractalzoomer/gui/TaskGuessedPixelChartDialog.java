...
public class TaskGuessedPixelChartDialog extends JDialog {
    ...
    private PieDataset createDataset(ArrayList<TaskStatistic> taskStatistics) {
        ...
        for(TaskStatistic stat : taskStatistics) {
            if(stat.getPixelsCalculated() >= 0 && stat.getPixelsCompleted() >= 0) {
                dataset.setValue("Task " + stat.getTaskId(), stat.getPixelsCompleted() - stat.getPixelsCalculated());
    ...
    private JFreeChart createChart(PieDataset dataset) {
        JFreeChart chart = ChartFactory.createPieChart(
                "Guessed Pixels Statistics",  // chart title
                dataset,            // data
                true,              // no legend
                true,               // tooltips
    ...
    public TaskGuessedPixelChartDialog(JFrame ptra2, ArrayList<TaskStatistic> taskStatistics) {
        super();

        int chart_width = 700;
        int chart_height = 700;
        setSize(chart_width, chart_height);
        setTitle("Guessed Pixels Statistics");
        setModal(true);
