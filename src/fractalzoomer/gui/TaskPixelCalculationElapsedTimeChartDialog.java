...
public class TaskPixelCalculationElapsedTimeChartDialog extends JDialog {
    private HashMap<Integer, Color> colors = new HashMap<>();

    private PieDataset createDataset(ArrayList<TaskStatistic> taskStatistics, long maxCalculationTime) {
        DefaultPieDataset dataset = new DefaultPieDataset();

        for(TaskStatistic stat : taskStatistics) {
            if (maxCalculationTime > 0) {
                dataset.setValue("Task " + stat.getTaskId(), stat.getPixelCalculationTime());
    ...
    private JFreeChart createChart(PieDataset dataset) {
        JFreeChart chart = ChartFactory.createPieChart(
                "Pixel Calculation Elapsed Time Statistics",  // chart title
                dataset,            // data
                true,              // no legend
                true,               // tooltips
    ...
    public TaskPixelCalculationElapsedTimeChartDialog(JFrame ptra2, ArrayList<TaskStatistic> taskStatistics, long maxCalculationTime) {
        super();

        int chart_width = 700;
        int chart_height = 700;
        setSize(chart_width, chart_height);
        setTitle("Pixel Calculation Elapsed Time Statistics");
        setModal(true);
        ...
        JFreeChart chart = createChart(createDataset(taskStatistics, maxCalculationTime));
