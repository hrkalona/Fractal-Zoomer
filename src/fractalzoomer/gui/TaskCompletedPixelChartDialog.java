...
public class TaskCompletedPixelChartDialog extends JDialog {
    ...
    private PieDataset createDataset(ArrayList<TaskStatistic> taskStatistics) {
        ...
        for(TaskStatistic stat : taskStatistics) {
            if(stat.getPixelsCompleted() >= 0) {
                dataset.setValue("Task " + stat.getTaskId(), stat.getPixelsCompleted());
    ...
    private JFreeChart createChart(PieDataset dataset) {
        JFreeChart chart = ChartFactory.createPieChart(
                "Completed Pixels Statistics",  // chart title
                dataset,            // data
                true,              // no legend
                true,               // tooltips
    ...
    public TaskCompletedPixelChartDialog(JFrame ptra2, ArrayList<TaskStatistic> taskStatistics) {
        super();

        int chart_width = 700;
        int chart_height = 700;
        setSize(chart_width, chart_height);
        setTitle("Completed Pixels Statistics");
        setModal(true);
