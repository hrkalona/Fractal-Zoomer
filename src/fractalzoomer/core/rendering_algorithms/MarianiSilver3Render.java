...
public class MarianiSilver3Render extends MarianiSilverRender {
    ...
    protected void render(int image_width, int image_height, boolean polar) throws StopSuccessiveRefinementException {
        ...
        do {
            ...
            if (slice_TOy == image_height) {
                slice_TOy--;
            }

            if (slice_TOx == image_width) {
                slice_TOx--;
            }
            ...
            if (whole_area) {
                for (y = slice_FROMy + 1, loc = y * image_width + slice_FROMx, loc2 = y * image_width + slice_TOx; y <= slice_TOy - 1; y++, loc += image_width, loc2 += image_width) {
                    if (isNotTheSame(loc, temp_starting_pixel_color, temp_starting_value) || isNotTheSame(loc2, temp_starting_pixel_color, temp_starting_value)) {
                        whole_area = false;
                        break;
                    }
            ...
            if (!whole_area) {
                ...
                if (canSubDivide(xLength, yLength)) {
                    if(getSplitMode(currentSquare.iteration) == 1) {
    ...
    private int getSplitMode(int iteration) {
        if(TaskRender.SUCCESSIVE_REFINEMENT_SQUARE_RECT_SPLIT_ALGORITHM == 1) {
            return ((iteration + 1) % 2); //1, 0, 1, 0,...
        }
        else if(TaskRender.SUCCESSIVE_REFINEMENT_SQUARE_RECT_SPLIT_ALGORITHM == 2) {
            return iteration % 2; // 0, 1, 0, 1,...
        }
        else if(TaskRender.SUCCESSIVE_REFINEMENT_SQUARE_RECT_SPLIT_ALGORITHM == 3) {
            //1, 0, 0, 1, 1, 0, 0, 1, 1
        ...
        else if(TaskRender.SUCCESSIVE_REFINEMENT_SQUARE_RECT_SPLIT_ALGORITHM == 4) {
            //0, 1, 1, 0, 0, 1, 1, 0, 0,...
            int val = iteration % 2;
