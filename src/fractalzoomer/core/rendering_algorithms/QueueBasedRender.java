...
public abstract class QueueBasedRender extends TaskRender {
    ...
    public static boolean USE_GLOBAL_QUEUE = true;
    ...
    private static ExpandingQueueSquare[] allQueuesSquare;
    private static ExpandingQueueSquare globalQueueSquare;
    ...
    public static void initStatic(int num_tasks) {
        if (USE_GLOBAL_QUEUE) {
            globalQueueSquare = new ExpandingQueueSquare(RENDER_USING_DFS ? INIT_QUEUE_SIZE : INIT_QUEUE_SIZE2);
        } else {
    ...
    protected void createQueueSquare() {
        ...
        perform_work_stealing = WORK_STEALING_ENABLED && (USE_GLOBAL_QUEUE || allQueuesSquare.length > 1);
    ...
    private static Square getNextSquareFrom(ExpandingQueueSquare queue) {
        if (RENDER_USING_DFS) {
            return queue.last();
        }
        ...
        if(perform_work_stealing) {
            ...
            do {
                if (USE_GLOBAL_QUEUE) {
                    ...
                    synchronized (globalQueueSquare) {
                        if (!globalQueueSquare.isEmpty()) {
                            return getNextSquareFrom(globalQueueSquare);
                        }
                    }
                } else {
    ...
    private void stop() {
        ...
        synchronized (USE_GLOBAL_QUEUE ? globalQueueSquare : allQueuesSquare) {
        ...
        if(perform_work_stealing) {
            if (USE_GLOBAL_QUEUE) {
                if (square.isSmall()) {
                    ...
                    synchronized (globalQueueSquare) {
                        globalQueueSquare.enqueue(square);
                        if (threadsWaiting > 0) {
                            globalQueueSquare.notify();
                        }
                    }
                }
            } else {
        ...
        if(perform_work_stealing) {
            if (USE_GLOBAL_QUEUE) {
                if (square1.isSmall()) {
                    ...
                    synchronized (globalQueueSquare) {
                        globalQueueSquare.enqueue(square1);
                        globalQueueSquare.enqueue(square2);
                        globalQueueSquare.enqueue(square3);
                        globalQueueSquare.enqueue(square4);
