...
public class DivideAndConquerColorsAndIterationDataDraw extends ThreadDraw {
    ...
    protected void drawIterations(int image_size, boolean polar) {
        ...
        do {
            ...
            for (loc = slice_FROMy * image_size + x, loc2 = slice_TOy * image_size + x; x <= slice_TOx; x++, loc++, loc2++) {
                if (image_iterations[loc] != temp_starting_value || image_iterations[loc2] != temp_starting_value
                || rgbs[loc] != temp_starting_pixel_color || rgbs[loc2] != temp_starting_pixel_color) {
                    whole_area = false;
                    break;
                }
            }
    ...
    protected void drawFastJulia(int image_size, boolean polar) {
        ...
        do {
            ...
            int loc2;
            for (loc = slice_FROMy * image_size + x, loc2 = slice_TOy * image_size + x; x <= slice_TOx; x++, loc++, loc2++) {
                if (image_iterations_fast_julia[loc] != temp_starting_value || image_iterations_fast_julia[loc2] != temp_starting_value
                        || rgbs[loc] != temp_starting_pixel_color || rgbs[loc2] != temp_starting_pixel_color) {
                    whole_area = false;
                    break;
                }
