...
public class MagnetPataki3 extends MagnetPatakiType {
    ...
    public void function(Complex[] complex) {
        ...
        Complex zcube = complex[0].cube();
        complex[0] = zcube.plus(complex[1]).divide_mutable(zcube.sub(1));

    }

    @Override
    public void function(GenericComplex[] complex) {
        GenericComplex zcube = complex[0].cube();
        if(complex[0] instanceof BigComplex) {
            complex[0] = zcube.plus(complex[1]).divide(zcube.sub(MyApfloat.ONE));
        }
        else {
            complex[0] = zcube.plus(complex[1]).divide_mutable(zcube.sub(1));
        }
    ...
    //-(3*(C + 1)*Z^2*z + 3*(C + 1)*Z*z^2 + (C + 1)*z^3 - (Z^3 - 1)*c)
    // /
    // (Z^6 + (Z^3 - 1)*z^3 - 2*Z^3 + 3*(Z^4 - Z)*z^2 + 3*(Z^5 - Z^2)*z + 1)
    @Override
    public Complex perturbationFunction(Complex z, Complex c, int RefIteration) {
        ...
        Complex zcubes1 = getExpressionValue(referenceData.PrecalculatedTerms[0], RefIteration, Z);

        Complex temp = Zsqr.times(3).plus_mutable(Z.times(3).plus_mutable(z).times_mutable(z)).times_mutable(z); //3*Z^2 + (3*Z + z)*z)*z

        Complex num = temp.times(Cp1)
                .sub_mutable(zcubes1.times(c)).negative_mutable();

        Complex denom = temp.times(zcubes1)
    ...
    public MantExpComplex perturbationFunction(MantExpComplex z, MantExpComplex c, int RefIteration) {
        ...
        MantExpComplex zcubes1 = getExpressionDeepValue(referenceDeepData.PrecalculatedTerms[0], RefIteration, Z);

        MantExpComplex temp = Zsqr.times(MantExp.THREE).plus_mutable(Z.times(MantExp.THREE).plus_mutable(z).times_mutable(z)).times_mutable(z); //3*Z^2 + (3*Z + z)*z)*z

        MantExpComplex num = temp.times(Cp1Deep)
                .sub_mutable(zcubes1.times(c)).negative_mutable();

        MantExpComplex denom = temp.times(zcubes1)
    ...
    public Complex perturbationFunction(Complex z, ReferenceData data, int RefIteration) {
        ...
        Complex zcubes1 = getExpressionValue(data.PrecalculatedTerms[0], RefIteration, Z);
    ...
    public MantExpComplex perturbationFunction(MantExpComplex z, ReferenceDeepData data, int RefIteration) {
        ...
        MantExpComplex zcubes1 = getExpressionDeepValue(data.PrecalculatedTerms[0], RefIteration, Z);
    ...
    protected SerializableFunction[] getPrecalculatedTermsFunctions(Complex c) {
        SerializableFunction<Complex, Complex> f1 = x -> x.cube().sub_mutable(1);
        SerializableFunction<Complex, Complex> f2 = x -> {Complex temp = x.cube(); return temp.sub(2).times_mutable(temp).plus_mutable(1);};
        return new SerializableFunction[] {f1, f2};
    }

    @Override
    protected SerializableFunction[] getPrecalculatedTermsFunctionsDeep(MantExpComplex c) {
        SerializableFunction<MantExpComplex, MantExpComplex> f1 = x -> x.cube().sub_mutable(MantExp.ONE);
        SerializableFunction<MantExpComplex, MantExpComplex> f2 = x -> {MantExpComplex temp = x.cube(); return temp.sub(MantExp.TWO).times_mutable(temp).plus_mutable(MantExp.ONE);};
        return new SerializableFunction[] {f1, f2};
    }
    ...
    protected GenericComplex[] precalculateReferenceData(GenericComplex z, GenericComplex c, NormComponents normData, Location loc, int bigNumLib, boolean lowPrecReferenceOrbitNeeded, boolean deepZoom, ReferenceData referenceData, ReferenceDeepData referenceDeepData, int iterations, Complex cz, MantExpComplex mcz) {
        GenericComplex zcube;
        if(normData != null) {
            zcube = z.cubeFast_mutable(normData);
        }
        else {
            zcube = z.cube();
        }
        ...
        if(bigNumLib != Constants.BIGNUM_APFLOAT) {
            ...
            preCalc = zcube.sub(MyApfloat.ONE);
        }

        GenericComplex preCalc2;
        if(bigNumLib != Constants.BIGNUM_APFLOAT) {
            preCalc2 = zcube.sub(2).times_mutable(zcube).plus_mutable(1);
        }
        else {
            preCalc2 = zcube.sub(MyApfloat.TWO).times(zcube).plus(MyApfloat.ONE);
        }
        ...
        return new GenericComplex[] {zcube, preCalc};
    }
    ...
    public Complex function(Complex z, Complex c) {
        ...
        return zcube.plus(c).divide_mutable(zcube.sub(1));
    }

    @Override
    public MantExpComplex function(MantExpComplex z, MantExpComplex c) {
        MantExpComplex zcube = z.cube();
        return zcube.plus(c).divide_mutable(zcube.sub(MantExp.ONE));
    }
