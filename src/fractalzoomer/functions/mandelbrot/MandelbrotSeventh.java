...
public class MandelbrotSeventh extends Julia {
    ...
    public void function(GenericComplex[] complex) {
        if(burning_ship) {
            GenericComplex tempz = complex[0].abs_mutable();
            complex[0] = tempz.fifth().times(tempz.square()).plus_mutable(complex[1]);
        } else {
            complex[0] = complex[0].fifth().times(complex[0].square()).plus_mutable(complex[1]);
        }
    }
