...
import fractalzoomer.core.approximation.series_approximation.MandelbrotFourthApproximation;
...
//                    .plus_mutable(DeltaSubN.times4().times_mutable(X.cube()));
    ...
    public double getBlaR(Complex Z, double epsilon) {
        return (2.0 / 3.0) * Z.hypot() * epsilon;
    }

    @Override
    public MantExp getBlaR(MantExpComplex Z, MantExp epsilon) {
        return Z.hypot().multiply_mutable(2.0 / 3.0).multiply_mutable(epsilon);
    }
    ...
    protected void calculateSeries(Apfloat dsize, boolean deepZoom, Location loc, JProgressBar progress) {
        sa = new MandelbrotFourthApproximation();
